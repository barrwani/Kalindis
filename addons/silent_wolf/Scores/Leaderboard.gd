...
func _start():
	...
	if len(scores) > 0 && up == 0: 
		add_loading_scores_message()
		clear_board()
		render_board(scores, local_scores)
		hide_message()
	elif len(scores) > 0 && up > 0:
		add_loading_scores_message()
		yield(SilentWolf.Scores.get_score_position(6*up, "Guided"), "sw_position_received")
		print("pos", SilentWolf.Scores.position)
		yield(SilentWolf.Scores.get_scores_around(3*up, 6, "Guided"), "sw_scores_around_received")
		scores = SilentWolf.Scores.scores_above
		hide_message()
		#For this to work, we need to clear the old scores by 'queue_free()'ing their score item container nodes and resetting list_index
		clear_board()
		render_board(scores, local_scores)
	else:
		# use a signal to notify when the high scores have been returned, and show a "loading" animation until it's the case...
		add_loading_scores_message()
		yield(SilentWolf.Scores.get_high_scores("Guided"), "sw_scores_received")
		hide_message()
...
func clear_board():
	for i in range(0, $"Board/HighScores/ScoreItemContainer".get_child_count()):
		$"Board/HighScores/ScoreItemContainer".get_child(i).queue_free()
	list_index = 0

func merge_scores_with_local_scores(scores, local_scores, max_scores=6):
	if local_scores:
...
func add_item(player_name, score):
	if list_index < 6:
		...
		if up == 0:
			...
			item.get_node("PlayerName").text = str(list_index+6*up) + str(". ") + player_name
