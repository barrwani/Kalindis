...
func _start():
	...
	if len(scores) > 0 && up == 0: 
		...
		add_loading_scores_message()
		yield(SilentWolf.Scores.get_scores_around(12*up, 6, "HardMode"), "sw_scores_around_received")
		scores = SilentWolf.Scores.scores_below
		hide_message()
		...
		add_loading_scores_message()
		yield(SilentWolf.Scores.get_high_scores("HardMode"), "sw_scores_received")
		hide_message()
...
func add_item(player_name, score):
	if list_index < 6:
		...
		if up == 0:
			...
			item.get_node("PlayerName").text = str((list_index+6*up)-1) + str(". ") + player_name
